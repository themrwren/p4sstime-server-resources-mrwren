...
Action Timer_DisplayStats(Handle timer)
{
    ...
    // example values:
    // red% = 54.53% (0.5453) becomes 5453
    // blu% = 45.47% (0.4547) becomes 4547
    ...
    for (int x = 1; x < MaxClients + 1; x++)
    {
        ...
        PrintToConsole(x, "[PASS] BLU possession time in ticks: %d", iRedBallTime);
        PrintToConsole(x, "[PASS] RED possession time in ticks: %d", iBluBallTime);
        if (!isStv)
        {
